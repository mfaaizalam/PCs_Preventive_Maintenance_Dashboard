...
import {
  fetchMaintenanceRetentionStatus,
...
export default function RetentionReminderBanner() {
  ...
  if (
    ...
    status.affected_record_count === 0
  ) {
    return null;
  }
        ...
        <div>
          <p className="text-sm font-semibold text-amber-900">
            {status.days_remaining <= 0
              ? "Maintenance data is being purged today."
              : `Maintenance data will be deleted in ${status.days_remaining} days.`}
          </p>

          <p className="mt-0.5 text-sm text-amber-800">
            {status.affected_record_count} records from{" "}
            {status.period_label} will be affected. Please export the
            records if you need to keep them.
          </p>
        </div>
      </div>
